import React, { Fragment, useState, useContext, use } from "react";
  ...
  const formSubmit = async () => {
    ...
    try {
      ...
      if (responseData.error) {
        ...
        if (JSON.parse(localStorage.getItem("loginRequired"))) {
    ...
    <Fragment>
      ...
      <form className="space-y-4">
        ...
        <div className="flex flex-col space-y-2 md:flex-row md:justify-between md:items-center">
          <div>
            <input
              type="checkbox"
              id="rememberMe"
              className="px-4 py-2 focus:outline-none border mr-1 rounded-full"
            />
        ...
        <div
          ...
          className="font-medium px-4 py-2 text-white text-center cursor-pointer rounded-full"
        >
          Login
        </div>
        <div
